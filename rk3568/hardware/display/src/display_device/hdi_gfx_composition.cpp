...
{
    std::lock_guard<std::mutex> lock(mCompLayersMutex);
    ...
    for (auto &layer : layers) {
        if (layer == nullptr) {
            DISPLAY_LOGE("HdiGfxComposition::SetLayers layer is null");
            return DISPLAY_FAILURE;
        }
        if (!CanHandle(*layer)) {
            continue;
...
int32_t HdiGfxComposition::Apply(bool modeSet)
{
    ...
    for (uint32_t i = 0; i < mCompLayers.size(); i++) {
		...
		if (layer == nullptr) {
            DISPLAY_LOGD("HdiGfxComposition::Apply layer1 is null");			
		    return DISPLAY_FAILURE;
		}
    ...
    for (uint32_t i = 0; i < mCompLayers.size(); i++) {
        HdiLayer *layer = mCompLayers[i];
		if (layer == nullptr) {
            DISPLAY_LOGD("HdiGfxComposition::Apply layer2 is null");			
		    return DISPLAY_FAILURE;
		}		
        CompositionType compType = layer->GetCompositionType();
        switch (compType) {
